...
int main (void)
{
    ...
    Emios_Pwm_Ip_InitChannel(EMIOS_PWM_IP_VS_0_I0_CH14_CFG, &Emios_Pwm_Ip_VS_0_I0_Ch14);
    ...
    Emios_Pwm_Ip_SetDutyCycle(INSTANCE_0, CHANNEL_14, 0x200);

    /* Setup new duty cycle to the pin*/
    Emios_Pwm_Ip_SetDutyCycle(INSTANCE_0, CHANNEL_14, 0x4000);

    /* Using duty 0% and 100% to blink led */
    while(num_blink < NUM_BLINK_LED)
    {
        /* Led on when duty cycle is 100% */
        Emios_Pwm_Ip_SetDutyCycle(INSTANCE_0, CHANNEL_14, 0x5000);
        TestDelay(DELAY_TIMER);

        /* Led off when duty cycle is 0% */
        Emios_Pwm_Ip_SetDutyCycle(INSTANCE_0, CHANNEL_14, 0U);
        TestDelay(DELAY_TIMER);
        num_blink++;
    }

    /* De-Initialize Emios_Pwm_Ip driver */
    Emios_Pwm_Ip_DeInitChannel(INSTANCE_0, CHANNEL_14);
