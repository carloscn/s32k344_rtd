...
int main(void)
{
    ...
    Pwm_SetDutyCycle(EMIOS_0_CH_14, 0x200);
    Pwm_SetDutyCycle(FLEXIO_0_CH_1_D_2, 0x200);

    /* Increasing LED brightness using Pwm_SetDutyCycle*/
    Pwm_SetDutyCycle(EMIOS_0_CH_14, 0x7000);
    Pwm_SetDutyCycle(FLEXIO_0_CH_1_D_2, 0x7000);

    /* Using duty cycle 0% and 100% to Blink LED */
    while(num_blink < NUM_BLINK_LED)
    {
        /* Led on when duty cycle is 100% */
        Pwm_SetDutyCycle(EMIOS_0_CH_14, 0x8000);
        Pwm_SetDutyCycle(FLEXIO_0_CH_1_D_2, 0x8000);
        TestDelay(DELAY_TIMER);

        /* Led off when duty cycle is 0% */
        Pwm_SetDutyCycle(EMIOS_0_CH_14, 0x0000);
        Pwm_SetDutyCycle(FLEXIO_0_CH_1_D_2, 0x0000);
        TestDelay(DELAY_TIMER);
        num_blink++;
    }
