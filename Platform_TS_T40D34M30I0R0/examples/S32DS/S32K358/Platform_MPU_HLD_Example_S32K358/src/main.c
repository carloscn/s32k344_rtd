...
Platform_Mpu_M7_RegionConfigType testRegionConfig;
Mcl_DmaChannelTransferListType aDmaParamList[DMA_CH0_CONFIG_LIST_DIMENSION] =
{
    {MCL_DMA_CH_SET_SOURCE_ADDRESS, (uint32)&ua8DmaSrcData[0U]},
    {MCL_DMA_CH_SET_SOURCE_SIGNED_OFFSET, 4},
    {MCL_DMA_CH_SET_SOURCE_TRANSFER_SIZE, 2},

    {MCL_DMA_CH_SET_DESTINATION_ADDRESS, (uint32)&ua8DmadestData[0U]},
    {MCL_DMA_CH_SET_DESTINATION_SIGNED_OFFSET,4},
    {MCL_DMA_CH_SET_DESTINATION_TRANSFER_SIZE,2},

    {MCL_DMA_CH_SET_MINORLOOP_SIZE, (uint32) 32},
    {MCL_DMA_CH_SET_MAJORLOOP_COUNT, 1}
};

/* Status variables to be used in interrupt handler of MemManage fault */
volatile Platform_Mpu_M7_ErrorDetailsType ErrorDetails;
...
static void PerformDmaTransfer(void)
{
    ...
    Mcl_SetDmaChannelTransferList(DMA_LOGIC_CH_0, aDmaParamList, DMA_CH0_CONFIG_LIST_DIMENSION);
...
void MemManage_Handler(void)
{
    ...
    Platform_Mpu_M7_ErrorDetailsType ErrorDetailsTemp;
    /* Get error details */
    (void)Platform_Mpu_M7_GetErrorDetails(&ErrorDetailsTemp);
    ErrorDetails = ErrorDetailsTemp;
    u8MemFaultFlag = TRUE;
    S32_SCB->CFSR |= MPU_M7_IP_SCB_CFSR_MMFSR_MASK_WITHOUT_VALID;
    Platform_Mpu_M7_SetAccessRight(MPU_REGION_CACHE, MPU_M7_PRIV_RWX_UNPRIV_RWX);
    Platform_Mpu_M7_SetAccessRight(MPU_REGION_NOCACHE, MPU_M7_PRIV_RWX_UNPRIV_RWX);
    Platform_Mpu_M7_SetAccessRight(MPU_REGION_SHARE, MPU_M7_PRIV_RWX_UNPRIV_RWX);
    Platform_Mpu_M7_EnableRegion(MPU_REGION_TEST,FALSE);
}
...
void SetupRamCodeMpuRegion(void)
{
    ...
    Platform_Mpu_M7_SetRegionConfig(5, &testRegionConfig);
}
/**
...
*                 - Call PLATFORM_init to initialize 5 regions, with 5 regions configured as follows:
...
*                         - Call Platform_Mpu_M7_SetAccessRight to change permission access (MPU_M7_PRIV_UNPRIV_NONE) for the .mcal_data_no_cacheable section (MPU Region 2)
*                         - Verification Point: CPU read failed
*                 - 3. Access Write Data to Read-only MPU Region
*                         - Call Platform_Mpu_M7_SetAccessRight to change permission access (MPU_M7_PRIV_R_UNPRIV_R) for the .mcal_data_no_cacheable section (MPU Region 3)
...
*                         - Call Platform_Mpu_M7_SetRegionConfig() to change cache policy (CACHE_POLICY_W_BACK_WR_ALLOCATE)
*                         - Call Mcl_CacheCleanByAddr()
...
*                         - Call Platform_Mpu_M7_SetRegionConfig() to change cache policy (POLICY_W_THROUGH_NO_W_ALLOCATE)
*                         - Call Mcl_CacheCleanByAddr()
*                         - Write dummy value to do cache miss for first time accessing memory, make sure cache hit for next access time
*                         - Copy data by calling PerformDmaTransfer()
...
/* --------------------------------------- 1. Access Data To Read/Write/Execute MPU Region  ---------------------------------------*/
    ...
    /* ----------------------------------- 2. Access Read Data To Non-Read/Write MPU Region  ------------------------------------------*/
    Platform_Mpu_M7_SetAccessRight(MPU_REGION_NOCACHE, MPU_M7_PRIV_UNPRIV_NONE);
    /* Copy data from .mcal_data_no_cacheable section to .mcal_shared_data section */
    CopyArray(ua8SrcData, ua8DestData, DATA_LENGTH);
...
/* -------------------------------------- 3. Access Write Data to Read-only MPU Region  ------------------------------------------*/
    Platform_Mpu_M7_SetAccessRight(MPU_REGION_SHARE, MPU_M7_PRIV_R_UNPRIV_R);
    /* Copy data from .mcal_data_no_cacheable section to .mcal_shared_data section */
    CopyArray(ua8SrcData, ua8DestData, DATA_LENGTH);
...
/* ---------------- 4. Cache Coherence - Access Data to Write Back & Write Allocate MPU Region  -------------------------*/
    ...
    testRegionConfig.u8SubRegMask      = 0;
    testRegionConfig.bShareable        = false;
    Platform_Mpu_M7_SetRegionConfig(MPU_REGION_TEST, &testRegionConfig);
    Mcl_CacheCleanByAddr(MCL_CACHE_CORE, MCL_CACHE_DATA, TRUE, (uint32)(&ua8DmadestData), 32);
    ...
    /* Calling Mcl_CacheInvalidateByAddr() to result in a cache miss when CPU tries to read in the next access time */
    Mcl_CacheInvalidateByAddr(MCL_CACHE_CORE,MCL_CACHE_DATA, (uint32)(&ua8DmadestData), 32);
